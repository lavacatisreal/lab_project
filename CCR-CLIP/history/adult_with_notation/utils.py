...
def get_data_package():
    ...
    for dataset_root in config['train_dataset'].split(','):
        print(f"\033[32mDetect dataset: {dataset_root}\033[0m")
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset_total, batch_size=config['batch'], shuffle=True, num_workers=16,
    )
    ...
    test_dataloader = torch.utils.data.DataLoader(
        test_dataset_total, batch_size=config['batch'], shuffle=False, num_workers=16,
    )
